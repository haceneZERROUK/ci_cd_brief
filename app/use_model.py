...
def use_model(new_movies : list[dict]) :
    ...
    for new_movie in new_movies:
        for feature in expected_fields:
            if feature not in new_movie.keys():
                return "Error 1"

    expected_types = {
        "fr_title": str,
        "original_title": str,
        "released_date": str,        # ou datetime si tu la parses
        "released_year": str,
        "actor_1": str,
        "actor_2": str,
        "actor_3": str,
    ...
    for new_movie in new_movies:
        for key, value in expected_types.items():
            print(key, " ", value, " ", new_movie[key], "###########################################")
            if not isinstance(new_movie[key],value):
                return "Error 2"
            

    # # Boucle de vérification
    # for values in expected_types:
    #     expected_type = expected_types.get(values)
    #     if expected_type:
    #         value = data[key]
    #         assert isinstance(value, expected_type), f"Champ '{key}' a un type incorrect : attendu {expected_type}, obtenu {type(value)}"
    #     try:
    #         datetime.strptime(data["released_date"], "%Y-%m-%d")
    #     except ValueError:
    #         raise AssertionError("Le champ 'released_date' n'est pas au format 'YYYY-MM-DD'")
